...
while True:
    ...
    img_copied = imresize(img, (img_height, img_width))
    img_copied = np.array([img_copied])
    predictions = model.predict(img_copied)

    ratio_height = np.shape(img)[0] / img_height
    ratio_weight = np.shape(img)[1] / img_width
    for pred in predictions[0]:
        if (pred[1] != 0):
            xmin = int(pred[2] * ratio_weight)
            ymin = int(pred[3] * ratio_height)
            xmax = int(pred[4] * ratio_weight)
            ymax = int(pred[5] * ratio_height)
    


            cv2.rectangle(img, (xmin, ymin), (xmax, ymax), color=(255, 0, 0), thickness=2)

            cv2.putText(img, classes[int(pred[0])], (xmin, ymin - 8), cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.8, color=(0, 255, 255))
            ## works in python 3.6.5
