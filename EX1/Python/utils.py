...
def bilateral_filter(img, wsize, sigma_d, sigma_r):
    ...
    xv, yv = np.meshgrid(np.arange(-k, k+1), np.arange(-k, k+1))
    g = np.exp(-(xv*xv + yv*yv) / (2 * sigma_d ** 2))

    # Local function pointer variables for slight efficiency boost
    npexp_loc = np.exp
    npsum_loc = np.sum
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            img_patch = img[iMin:iMax+1, jMin:jMax+1]

            # Calculate domain kernel from pre-computed Gaussian distance weights
            distw = g[int(iMin-i+k):int(iMax-i+k+1), int(jMin-j+k):int(jMax-j+k+1)]

            # Compute Gaussian intensity weights (range kernel)
            rangew = npexp_loc(-(img_patch - img[i, j]) ** 2 / (2 * sigma_r ** 2))

            # Calculate bilateral weight function and the output pixel function
            totalw = rangew * distw
            output[i, j] = npsum_loc(totalw*img_patch) / np.sum(totalw)
    return output
